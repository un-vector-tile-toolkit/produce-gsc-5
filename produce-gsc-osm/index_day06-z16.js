...
const mbtilesDir = config.get('osm-l.mbtilesDir_day06') //edited 2020-02-12
const logDir = config.get('logDir')
const propertyBlacklist = config.get('osm-l.propertyBlacklist')
let conversionTilelist = config.get('day06Tilelist') //edited 2021-02-12
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-day06-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: day06 tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
