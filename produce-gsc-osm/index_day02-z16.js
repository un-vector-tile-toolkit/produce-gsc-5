...
const mbtilesDir = config.get('osm-l.mbtilesDir_day02') //edited 2020-02-12
const logDir = config.get('logDir')
const propertyBlacklist = config.get('osm-l.propertyBlacklist')
let conversionTilelist = config.get('day02Tilelist') //edited 2021-02-12
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-day02-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: day02 tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
